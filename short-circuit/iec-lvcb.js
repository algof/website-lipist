...
// ===== Auto-calculate: Making Peak (kA) = 2.5 * Service Breaking (kA) =====
const simServiceBreaking = document.getElementById('sim-service-breaking');
const simMakingPeak = document.getElementById('sim-making-peak');

if (simServiceBreaking && simMakingPeak) {
    simServiceBreaking.addEventListener('input', () => {
        const val = parseFloat(simServiceBreaking.value);
        if (!isNaN(val) && val !== 0) {
